...
`hearsay eval [--since] [--until] [--scope] [--json]` prints them over a window, as aggregates and scope and topic ids only: never a stance position, a topic name, L1 text or an L0 payload. The two metrics that need only the L2 ledgers come from them:
